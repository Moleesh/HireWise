  ...
  return (
    <div className="p-4 md:p-6 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl">
      <div className="flex items-center justify-between mb-4 md:mb-5">
        <h2 className="text-base md:text-lg font-semibold text-[var(--text-primary)]">{label}</h2>
        <button
      ...
      {items.length === 0 ? (
        <p className="text-[var(--text-quaternary)] text-sm py-8 text-center">No items yet</p>
      ) : (
        <div className="space-y-2">
          {items.map((item) => (
            <div
              ...
              onClick={() =>
                onNavigate(navigatePage, navigateData ? navigateData(item) : undefined)
              }
            >
